...
Note that at this point we are mentioning only the files/folders that are strictly "repository internal", i.e. they are not part of the lesson site built from the repository. The `config.yaml` file is kinda half-and-half: it isn't part of the built lesson, but its contents are relevant to the lesson site in that the configuration applies to the site. You do not have to dwell on the list too long, because these and other files and folders relevant to the lesson (including `config.yaml`) will be covered before the end of the training.
